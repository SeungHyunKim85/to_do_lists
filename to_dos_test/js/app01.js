(function () {
  ...
  function render() {
    ...
    // 선택된 필더를 기반으로 필터링된 todos 배열을 가져옵니다.
    ...
    filteredTodos.forEach(({ id, content, completed }) => {
      ...
      // input(체크 박스), label(할일 내용), i(삭제 버튼) 요소를 li 태그에 집어넣어줍니다.
    ...
    // 완료된 할 일 갯수를 넣어줍니다.
    document.querySelector('.completed-todos').textContent = todos.filter((todo) => todo.completed === true).length;

    // 완료되지 않은 할 일 갯수를 넣어줍니다.
    document.querySelector('.active-todos').textContent = todos.filter((todo) => todo.completed === false).length;
  }
  ...
  /* ADD EVENT HANDLER */
  // input에서 엔터를 눌렀다 띄었을 때 할 일을 추가해주는 이벤트 핸들러
  ...
  // checkbox가 토글되었을 때 할 일의 완료 상태를 토글해주는 이벤트 핸들러
  ...
  // 버튼을 클릭했을 때 모든 할 일의 완료 상태를 체크 상태에 따라 true 혹은 false로 바꿔주는 이벤트 핸들러
  ...
  // 버튼을 클릭했을 떄 완료된 할 일을 모두 지워주는 함수.
